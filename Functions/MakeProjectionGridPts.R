...
#' @param InclCoords should coordinates be included in data? (defaults to \code{TRUE}).
...
MakeProjectionGridPts  <- function(nxy, mesh, data, tag='pred', coordnames = c("X", "Y"), boundary=NULL, intercept=TRUE, InclCoords=TRUE) {
  ...
  if(InclCoords) NearestCovs@data[,colnames(NearestCovs@coords)] <- NearestCovs@coords
