...
#' @export
#' @import INLA
MakeMarkedPointsStackPts <- function(data, presences, tag="points", intercept=TRUE, mesh,
  ...
  NearestCovs@data <- data.frame(NearestCovs@data, ID = presences$ID, season = presences$season)
  ...
  stk.pp <- inla.stack(data=list(resp=rep(1,nrow(NearestCovs)),  ## Colin changed here
